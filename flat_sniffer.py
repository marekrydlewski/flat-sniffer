...
listings (status, price, unit number...) server-side per category. No
external dependencies - stdlib only.

Usage:
    python3 flat_sniffer.py            # fetch, diff against registry.json, print changes, save
    python3 flat_sniffer.py --json     # also print the full diff as JSON
    python3 flat_sniffer.py --quiet    # only print if there are changes
"""
import argparse
import json
import re
...
def diff_registries(old: dict, new: dict) -> list[dict]:
    ...
    for oid in sorted(new_ids - old_ids, key=lambda x: int(x)):
        ...
        events.append({
            ...
            "status": o["status"], "price": o["price"], "url": o["url"],
        })
    ...
    for oid in sorted(old_ids - new_ids, key=lambda x: int(x)):
        ...
        events.append({
            ...
            "category": o["category"], "unit": o["unit"],
            "last_status": o["status"], "price": o["price"], "url": o["url"],
...
def print_report(events: list[dict], new_registry: dict, first_run: bool) -> None:
    ...
    for e in events:
        if e["event"] == "status_change":
            print(f"  [STATUS] {e['category']} {e['unit']}: "
                  f"{e['old_status']} -> {e['new_status']}  ({e['url']})")
        elif e["event"] == "price_change":
            print(f"  [PRICE]  {e['category']} {e['unit']}: "
                  f"{e['old_price']} -> {e['new_price']}  ({e['url']})")
        elif e["event"] == "new_listing":
            print(f"  [NEW]    {e['category']} {e['unit']}: "
                  f"{e['status']} @ {e['price']}  ({e['url']})")
        elif e["event"] == "removed_from_listing":
            print(f"  [GONE]   {e['category']} {e['unit']}: "
                  f"was {e['last_status']} @ {e['price']} - {e['note']}  ({e['url']})")
...
def main():
    ...
    events = diff_registries(old_registry, new_registry)
