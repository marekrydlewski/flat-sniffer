...
.changelog-item {{ display:grid; grid-template-columns:90px 1fr; gap:10px; align-items:start; margin:8px 0 }} .changelog-item time {{ color:var(--muted); font-size:.8rem; padding:16px 0 }} details {{ margin-top:24px }} summary {{ cursor:pointer; color:var(--ink); font-weight:650; padding:12px 0 }} .range-label {{ display:block; color:var(--muted); font-size:.85rem; margin:5px 0 12px }} select {{ border:1px solid var(--line); border-radius:6px; padding:5px; background:white; font:inherit }}
...
<div class="category-stats">{summaries}</div><p class="updated">Pasek pokazuje udział: <span class="status-available">wolne</span> · <span class="status-reserved">rezerwacje</span> · <span class="status-sold">sprzedane</span>. Sprzedane obejmuje pozycje potwierdzone na stronie dewelopera.</p><section><h2>Historia zniknięć z oferty — ostatnie 21 dni</h2>{trends}<p class="updated">Zniknięcie z oferty zwykle oznacza sprzedaż lub wycofanie.</p></section>
<section><h2>Od ostatniego sprawdzenia</h2><div class="changes">{changes}</div>{full_changelog}</section><section><h2>Pozycje widoczne obecnie na stronie dewelopera</h2>{cards}</section>
